...
keywords[["font_size"]] <- new("keyword",name="font_size",type="size", in_out="in", value="8")
...
scale_y_continuous(sec.axis=sec_axis(trans = ~./max_yCom, name="percentage",labels=scales::percent_format(suffix = "")))+ scale_x_continuous(breaks = seq(0, max_xCom, by = 1),expand = c(0.01,0)) +theme(text = element_text(size=as.numeric(keywords[["font_size"]]@value))) +scale_color_manual(name = "Y series", values = c("Cumulative percentage" = "black"))+theme(legend.key = element_rect(fill = NA))+theme(
